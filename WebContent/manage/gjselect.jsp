...
function tijiao(){
	...
	for(j=0;j<=i;j++){
		var one=$("#one_"+i).val();
		var two=$("#two_"+i).val();
		var three=$("#three_"+i).val();
		var four=$("#four_"+i).val();
		json['one_'+i]=one;
		json['two_'+i]=two;
		json['three_'+i]=three;
		json['four_'+i]=four;
	...
	$.post(
			...
			function(data){
				...
				var th="<tr>"+
                    ...
                    "<th>操作</th>"+
                     "</tr>";
                     $(".result-tab").append(th);    
				for(var i=0;i<data.length;i++)
			     {
					var table ="<tr><td><input name='id[]' value='' type='checkbox'></td><td>"+i+1+"</td><td>" + data[i].Username + "</td><td>"
					+ data[i].Name + "</td><td>" + data[i].Sz_address + "</td><td>"
					+ data[i].Legal + "</td><td>" + data[i].Institution + "</td><td>"
					+ data[i].New_time + "</td> <td>" + data[i].State + "</td><td><a href='../A_look_text?id="+data[i].id+"' >查看详情</a></td></tr>" 
			        $(".result-tab").append(table);
			     }
			},
			"json"
	)
